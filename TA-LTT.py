...
# The objective is to maximize the number of successful transfer connections.
obj_1 = LinExpr(0)
for transfer_connection in transfer_connection_list:
    obj_1 += transfer_indicator[
        transfer_connection[0], int(transfer_connection[1][0]), int(transfer_connection[1][2]), int(
            transfer_connection[2][0]), int(
            transfer_connection[2][2])]

m.setObjective(obj_1, GRB.MAXIMIZE)
...
print()
print('The number of successful transfer connections: ' + str(int(obj_1.getValue())))
print()
...
remaining_distance_sum = 0
for od_pair in path_dict:
    for i in range(len(path_dict[od_pair])):
        remaining_distance_sum += demands.loc[od_pair] * shortest_path_indicator[od_pair][i] * \
                                  od_pair_path_remaining_distance[
                                      od_pair[0], od_pair[1], i + 1].X
# The total path remaining distance of passengers
print('The total path remaining distance of passengers: ' + str(int(remaining_distance_sum)) + 'd')
